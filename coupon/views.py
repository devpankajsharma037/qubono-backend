...
class StoreAdminView(viewsets.ViewSet):
    ...
    def storeCouponCreate(self,request):
        ...
        try:
            ...
            serializer  = CouponSaveSerializer(data=payLoad)
            if not serializer.is_valid():
                context["status"]   = False
                context["code"]     = status.HTTP_400_BAD_REQUEST
                context["message"]  = serializer.errors
    ...
    @checkRole()
    def storeCouponUpdate(self,request):
        context = {}
        try:
            payLoad         = request.data
            ...
            serializer = CouponSaveSerializer(couponObj,data=payLoad,partial=True)
            if not serializer.is_valid():
                context["status"]   = False
                context["code"]     = status.HTTP_400_BAD_REQUEST
                context["message"]  = serializer.errors
    ...
    @checkRole()
    def storeCouponDelete(self,request):
        context = {}
        try:
            payLoad         = request.data
            payLoad['user'] = payLoad['user']     = request.user.id
...
class CategoryAdminView(viewsets.ViewSet):
    ...
    def categoryUpdate(self,request):
        ...
        try:
            payLoad     = request.data
            payLoad = request.data
            serializer = CategoryUpdateValidateSerializer(data=payLoad)
            if not serializer.is_valid():
                context["status"]   = False
                context["code"]     = status.HTTP_400_BAD_REQUEST
                context["message"]  = serializer.errors
...
class SubCategoryAdminView(viewsets.ViewSet):
    ...
    def subCategoryCreate(self,request):
        ...
        try:
            ...
            payLoad['user'] = request.user.id
            payLoad['is_active']    = True
            serializer  = SubCategoryValidateSerializer(data=payLoad)
            if not serializer.is_valid():
                context["status"]   = False
                context["code"]     = status.HTTP_400_BAD_REQUEST
                context["message"]  = serializer.errors
            ...
            serializer = SubCategorySaveSerializer(data=payLoad)
            if not serializer.is_valid():
                context["status"]   = False
                context["code"]     = status.HTTP_400_BAD_REQUEST
                context["message"]  = serializer.errors
    ...
    def subCategoryUpdate(self,request):
        ...
        try:
            payLoad    = request.data
            serializer = SubCategoryUpdateValidateSerializer(data=payLoad)
            if not serializer.is_valid():
                context["status"]   = False
                context["code"]     = status.HTTP_400_BAD_REQUEST
                context["message"]  = serializer.errors
            ...
            try:
                ...
                context["status"]   = False
                context["code"]     = status.HTTP_404_NOT_FOUND
                context["message"]  = "Sub category not found."
                return Response(context, status=status.HTTP_404_NOT_FOUND)
        
            serializer = SubCategorySaveSerializer(category,data=payLoad,partial=True)
            if not serializer.is_valid():
                context["status"]   = False
                context["code"]     = status.HTTP_400_BAD_REQUEST
                context["message"]  = serializer.errors
    ...
    def singleSubCategory(self,request,uuid):
        ...
        try:
            ...
            serializer          = GetAllSubCategorySerializer(categoryObj)
            context["status"]   = True
            context["code"]     = status.HTTP_200_OK
            context["message"]  = "success"
            context["data"]     = serializer.data
